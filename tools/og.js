...
.brand{position:absolute;left:72px;bottom:44px;font-family:"Bricolage Grotesque",sans-serif;font-weight:800;font-size:22px}
...
function page(lang, t){
  ...
  return `<!doctype html><html><head><meta charset="utf-8">${css}</head><body><div class="og" style="--h:${HUE[t.phase]}"><div><span class="chip">${esc(L.phases[t.phase])}</span><h1>${esc(x.name)}</h1><p>${esc(x.desc)}</p></div>${Preview.renderPreview(LAYOUTS[lang][t.id], {class:'pv'})}</div><div class="brand">${esc(L.title)}</div><div class="url">workshop-compass.vercel.app</div></body></html>`;
}
const defaultPage = `<!doctype html><html><head><meta charset="utf-8">${css}</head><body><div class="og hero" style="grid-template-columns:1fr"><div><span class="chip" style="--h:222">61 product workshops · FigJam · free</span><h1>Workshop Compass</h1><p style="max-width:44ch;font-size:26px">The right product workshop, at the right stage, in the right order. Every board ready to run, with a how-to and a filled example.</p></div></div><div class="brand">Workshop Compass</div><div class="url">workshop-compass.vercel.app</div></body></html>`;
const jobs = [];
